...
public class CriminalControlNoticeRusTitle extends AbstractNoticeRusTitle {
    ...
    protected void setBodyAdditionalInfo(PdfPTable table, Form103 form103) {
        ...
        String demandTwoStr = String.format("     Верховный Суд Республики Казахстан извещает Вас о том, что в соответствии со ст.  492 УПК РК рассмотрение уголовного дела по ходатайству %s в отношении %s назначено на %s  по адресу: г. Астана, ул. Д. Кунаева, д.39 (Левый берег р. Ишим).", form103.getF6(), form103.getF7(), form103.getF8());
        Phrase demandTwo = new Phrase(demandTwoStr, FontUtil.openSansRegular(10));
        addCellParagraph(table, demandTwo, 1);

        String demandThreeStr = String.format("    Для ознакомления с электронным документом (копия ходатайства, протеста, представления) необходимо на сайте Верховного Суда в модуле «Просмотр судебных документов» ввести идентификационный номер ШПИ (в правом верхнем углу) и дату назначения судебного заседания, указанные в судебном извещении.");
        Phrase demandThree = new Phrase(demandThreeStr, FontUtil.openSansRegular(10));
        addCellParagraph(table, demandThree, 1);

        String demandFourStr = String.format("    В соответствии с действующим законодательством для применения фотосъемки, видеозаписи в зале судебного заседания, участникам судебного процесса необходимо заранее получить разрешение председательствующего до начала судебного разбирательства путем подачи заявления в электронном или письменном виде.");
